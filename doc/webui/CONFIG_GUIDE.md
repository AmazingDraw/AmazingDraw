...
（发行包内 `scripts/config.json` 已跟踪，默认 `~` / `/tmp`。安装后运行时权威副本为 `~/.openclaw/draw-cards/config.json`。）
...
* **`obsidian_vault_dir`**
  - [Obsidian](https://obsidian.md/) 库根目录。提交后把 prompt / 卡片写成 markdown，精选（骰子）从库里的 `灵感库/` 随机抽一篇。
  - 库根下约定：`灵感库/`（精选用的 `.md`）与 `attachments/`（同名 `.png`）。用 [Obsidian](https://obsidian.md/) 打开该目录即可管理。空路径时精选不可用。
...
* **`restrict_roles`**
  - 默认 `true`；WebUI：「限制角色」。打开后限制角色不进入角色库。关掉则恢复。
...
*文档更新：2026-08-05 — 移除废弃键 output_dir_fallback；出图两目录职责对照；WebUI /images 双挂载。*
